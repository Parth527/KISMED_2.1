...
class DataLoader():
    ...
    def extract_heartbeats(self, signal, sampling_rate):
        ...
        heartbeats, rpeaks = bio.signals.ecg.extract_heartbeats(signal=filtered,
                                                                ...
                                                                before=0.2,
                                                                after=0.4)
        return (np.mean(heartbeats,axis=0), rpeaks, filtered)
    ...
    def process_signals(self, signals, sampling_rate, save=False, save_name='ECG_heartbeats.pkl'):
    ...
    def prepare_input_challenge(self, dataset, save=False, normalize=False, save_name=('X_train_af.pkl', 'y_af.pkl')):
        ...
        for sg_id in dataset.keys():
            ...
            if (lb != '~') & (lb != 'O'):  # discard noise ECG which can't be identify
                signal = dataset[sg_id][1]
                avged_signal = np.mean(signal, axis=0)

                if normalize:
                    hb, _ = self.normalize(hb)

                X_train.append(avged_signal)
                y.append(lb)
